...
Begin {
    ...
    Function Get-ClientVersion {
        try {
            if ($PowerShellVersion -ge 6) { [Version]$obj = (Get-CimInstance -Namespace root/ccm SMS_Client -ErrorAction Stop).ClientVersion }
            else { [Version]$obj = (Get-WmiObject -Namespace root/ccm SMS_Client -ErrorAction Stop).ClientVersion }
        }
        catch { [Version]$obj = "0.0.0.0" }
        finally { Write-Output $obj }
    }
    ...
    Function Test-DNSConfiguration {
        ...
        switch ($obj) {
            $false {
                ...
                if ($fix -eq "true") {
                    $text = 'DNS Check: FAILED. IP address published in DNS does not match IP address on local machine. Trying to resolve by registerting with DNS server'
...
Process {
    ...
    <#
    Write-Verbose 'Validate that ConfigMgr client does not have CcmSQLCE.log and are not in debug mode'
    ...
    $log.ClientVersion = (Get-ClientVersion).ToString()
